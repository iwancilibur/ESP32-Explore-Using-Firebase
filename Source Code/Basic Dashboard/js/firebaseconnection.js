  ...
  const jarak = document.getElementById('jarak'); //mengambil data dari parameter sensor
  const dbRef4 = firebase.database().ref().child('jarak'); //mengambil data dari firebase database dengan nama sensor
  dbRef4.on('value', snap => jarak.innerText = snap.val()); //menaruh nilai sensor kedalam kolom yang sudah disediakan
  ...
  var relay4 = db.ref('relay4'); //membuat variabel bernama relay4 dengan berisikan relay4
      relay4.on('value', function(snapshot) { //membuat fungsi isi dari variabel relay4
        console.log("relay4: "+snapshot.val()); //mengambil nilai console log relay4
        if(snapshot.val()) document.querySelector("#relay4").style.background = 'rgb(255, 100, 100)'; //jika relay diberi inputan 1 maka akan berubah warna background menjadi biru
        if(!snapshot.val()) document.querySelector("#relay4").style.background = 'transparent'; //jika relay diberi inputan 0 maka akan berubah warna background menjadi putih transparant
        document.querySelector("#Lampu4 > input").checked = snapshot.val(); //memberikan informasi bahwa lampu3 memberikan inputan
      }); //simbol untuk menutup fungsi
